...
from models.layers.spatial_conv import SpatialConvOrderK
from models.layers.gcrnn import GCGRUCell
from models.layers.spatial_attention import SpatialAttention
from utils.functions import reverse_tensor, str_to_bool
...
class BiGRIL(nn.Module):
    ...
    def forward(self, x, adj, mask=None, u=None, cached_support=False):
        ...
        *bwd_res, _ = self.bwd_rnn(rev_x, supp, mask=rev_mask, u=rev_u, cached_support=cached_support)
        ...
        if self.back:
            predictions = torch.stack([bwd_out, bwd_pred], dim=0)
